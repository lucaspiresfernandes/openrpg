        ...
        <>
            <Row className='mt-4 mb-2 justify-content-center'>
                <PlayerAvatarImage statusID={playerAttributeStatus.find(stat => stat.value)?.AttributeStatus.id}
                    onClick={() => setAvatarModalShow(true)} playerStatus={playerAttributeStatus} />
            ...
            <EditAvatarModal attributeStatus={playerAttributeStatus.map(stat => stat.AttributeStatus)} show={avatarModalShow}
                onHide={() => setAvatarModalShow(false)} onUpdate={() => setPlayerStatus([...playerAttributeStatus])} />
